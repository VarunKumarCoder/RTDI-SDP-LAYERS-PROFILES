...
public class MySqlEmployeeDaoImpl implements IEmployeeDao {
	private static final String GET_EMPYS_BY_DESG = "SELECT EMPNO, ENAME,JOB,SAL,DEPTNO FROM EMP WHERE JOB IN(?,?,?) ORDER BY JOB";
	@Autowired
	...
	public List<Employee> getEmployeesByDesignation(String desg1, String desg2, String desg3) throws Exception {
		...
		try (Connection con = ds.getConnection(); PreparedStatement ps = con.prepareStatement(GET_EMPYS_BY_DESG)) {
